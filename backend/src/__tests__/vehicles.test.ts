...
describe('Vehicles CRUD', () => {
  ...
  beforeAll(async () => {
    ...
    const adminEmail = `admin-vehicles-${timestamp}@automanager-test.com`;
    ...
    const companyResp = await agent.post('/api/service-company').send({
      ...
      phone: '0888123456',
      email: `garage-vehicles-${timestamp}@automanager-test.com`,
    });
    ...
    const clientResp = await agent.post('/api/clients').send({
      ...
      lastName: 'Client',
      phone: '0888000111',
      email: `client-vehicles-${timestamp}@automanager-test.com`,
    });
