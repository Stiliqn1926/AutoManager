...
export const updateOrder = async (
  ...
  try {
    ...
    // Cannot mark order as paid before invoice exists
    ...
    const invoice = await prisma.$transaction(async (tx: any) => {
      const createdInvoice = await tx.invoice.create({
        data: {
          ...
          total,
          issueDate: new Date(),
          isPaid: isAlreadyCompleted,
          paidDate: isAlreadyCompleted ? new Date() : null,
          serviceCompanyId: order.serviceCompanyId,
        },
      });
      ...
      await tx.order.update({
        where: { id },
        data: {
          status: isAlreadyCompleted ? 'COMPLETED' : 'READY',
          ...(isAlreadyCompleted && {
            completedDate: order.completedDate || new Date(),
          }),
        },
      });
